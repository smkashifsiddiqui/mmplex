...
if($_SESSION['user']->capabilities != 'null'){
	...
	if(in_array('cancel_concession', $user_capabilities)){
...
<div class="row">
		<ol class="breadcrumb">
		  <li><a href="concession.php">Home</a></li>
...
<div class="row form-header">
	  ...
	  <div class="row userLoginPanel">
		...
		if (isset($_POST['login'])) {
			...
			if(in_array('cancel_concession', $cancel_capability)){
			?>
				...
				<div class="row">
					<div class="col-md-12">	
						<?php 
						$concession = new concession();
						$results = $concession->get_concession_orders();
						...
						<table border="1" id="example" cellpadding="0" cellspacing="0" class="table table-hover tableView admin-table">
							...
							<tbody class="searchable">
								...
								foreach($results as $res){
										...
										echo '<tr>';
										echo '<td>'. $res->concession_order_id.'</td>';
										echo '<td>'. $res->user_fname.'</td>';
										echo '<td>'. $res->concession_order_amount .'</td>';
										echo '<td>'. date('d-M-y h:i', strtotime($res->concession_order_date)).'</td>';
										echo '<td class="alignCenter"><p>Remarks</p><textarea class="remarks"></textarea><br/><br/>';
										echo '<input type="hidden" class="id_person" value="'.$res->concession_order_user.'"><input type="hidden" class="id_to_delete" value="'.$res->concession_order_id.'"><a href="#" class="edit_btn img_delete" data-toggle="modal" data-target=".delete_confirm_modal">Cancel</a></td>';
										echo '<td class="alignCenter"><a href="reprint_concession.php?total='.$res->concession_order_amount.'&order_id='.$res->concession_order_id.'" class="edit_btn " data-toggle="modal" data-target="">Reprint</a></td>';
										echo '</tr>';
...
$(".container").on('click', '.img_delete',function() {
	...
	$(".modal-footer").on('click', '.btn_yes',function() {
		...
		$.post('ajax.php', {'to_delete': id_to_delete,'remarks':comment,'pers': id_person, 'action': 'delete_con'}, function(data) {
			console.log(data);
			if(data == true){
				location.reload();
			}else{
