...
<div class="row form-header">
	  ...
	  <div class="row userLoginPanel">
		<div class="col-md-6 col-md-offset-3">
			...
			<label for="max_record" class="" style=" font-size: 13px;margin-left: 5px;">Max Record</label>
		    <input type="text" name="max_record" id="max_date" class="form-control" placeholder="1000" required=""  style="margin-bottom: 15px; padding: 5px 12px;height: 40px;background: white;color: black;border: 0;">
						...
						<table border="1" id="example" cellpadding="0" cellspacing="0" class="table table-hover tableView admin-table">
							...
							<tbody class="searchable">
								...
								if(1 == 1){
										...
										echo '<input type="hidden" class="id_person" value="'.$cancel_id.'"><input type="hidden" class="id_to_delete" value="'.$res->printed_ticket_booking_id.'"><a href="#" class="edit_btn img_delete" data-toggle="modal" data-target=".delete_confirm_modal">Cancel</a></td>';
										echo '</tr>';
											}
								}}
								?>
...
$(".container").on('click', '.img_delete',function() {
	...
	$(".modal-footer").on('click', '.btn_yes',function() {
		...
		$.post('ajax.php', {'to_delete': id_to_delete,'remarks':comment,'pers': id_person, 'action': 'delete_printed_ticket'}, function(data) {
			console.log(data);
			if(data == true){
				location.reload();
			}else{
