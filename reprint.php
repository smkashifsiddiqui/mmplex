...
$print_sess = $_SESSION['reprint_ses'];
$logo_result = $booking->get_logo();

uasort($print_sess, function($a, $b) {
    return strnatcmp($a['seat_num'], $b['seat_num']);
});
foreach($print_sess as $print_sess_v){
	
	$booking_detail = $booking->get_reprinted_details($print_sess_v['shw_id'], $print_sess_v['seat_num']);
	?>
