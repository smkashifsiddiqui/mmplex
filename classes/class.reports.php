...
/*	
public function get_weekly_shows($r_movie_id){
	
	$this->force_select_all();
	$this->right_join('movies', 'm', 'm.movie_id = showtimes.showtime_movie_id');
	$this->where('showtime_movie_id',$r_movie_id);
	$this->select(array('count(showtimes.showtime_id)' => 'total_show'));
	$this->group_by('showtimes.showtime_date');
	$this->order_by('showtimes.showtime_date', 'asc');
	$this->from($this->table_showtime);
	return $this->all_results();
...
public function get_weekly_shows($r_movie_id){
	...
	$this->force_select_all();
	$this->inner_join('movies', 'm', 'm.movie_id = showtimes.showtime_movie_id');
...
public function get_movie_by_id($r_movie_id){
		$this->where('movie_id',$r_movie_id);
		$this->from($this->table_movie);
		return $this->result();
}

public function get_total_sale_by_id($shw_id){
		
		$this->select(array('sum(bookings.booking_price)' => 'total_tickets'));
		$this->where('booking_cancel','no');
		$this->where('booking_showtime_id',$shw_id);
		$this->from('bookings');
		return $this->result();
}

public function insert_show_profit($value,$profit){
		
		$data['show_sales']   = $profit;
		$this->where('showtime_id', $value);
		$this->update('showtimes', $data);
		    if (!$this->row_count()) {
				return false;
			}
    ...
    public function get_cash_in_hand($s_date){
	   ...
	   //print_f($s_date);
		...
		$this->where('booking_ts', array($start_date,$end_date), 'BETWEEN');
		 ...
		 $this->inner_join('screens', 'sc', 'sc.screen_id = s.showtime_screen_id');
		 $this->group_by(array('bookings.booking_user', 'bookings.booking_screen'));
		 //$this->group_by('bookings.booking_screen');
