...
*/
class profit_n_loss extends database
{
	private $table_name;
	...
	public function concession_total($r_start_date,$r_end_date)
	{
		$r_start_date1 = strtotime($r_start_date);
		$r_end_date1 = strtotime($r_end_date);
		$s_date = date("Y-m-d", $r_start_date1);
		$e_date = date("Y-m-d", $r_end_date1);
		//print_f($s_date);
		//print_f($e_date);
		$this->where('concession_order_date', array($s_date,$e_date), 'BETWEEN');
		$this->where('concession_order_cancel','no');
		$this->select(array('SUM(concession_order.concession_order_amount)' => 'total_amount'));	  	
		$this->from($this->con_order);
		return $this->result();
	}
	
	
	public function expense_total($r_start_date,$r_end_date)
	{
		$r_start_date1 = strtotime($r_start_date);
		$r_end_date1 = strtotime($r_end_date);
		$s_date = date("Y-m-d", $r_start_date1);
		...
		//print_f($s_date);
		//print_f($e_date);	
		$this->where('expense_date', array($s_date,$e_date), 'BETWEEN');
		$this->select(array('SUM(expenses.expense_amount)' => 'total_amount'));	  	
		$this->from($this->expenses);
		return $this->result();
	}
	
	public function payroll_total($r_start_date,$r_end_date)
	{
		$r_start_date1 = strtotime($r_start_date);
		$r_end_date1 = strtotime($r_end_date);
		$s_date = date("Y-m-d", $r_start_date1);
		...
		//print_f($s_date);
		//print_f($e_date);
		$this->where('payroll_date', array($s_date,$e_date), 'BETWEEN');
		$this->select(array('SUM(payrolls.payroll_amount)' => 'total_amount'));	  	
		$this->from($this->payrolls);
		return $this->result();
	}
	
	public function get_shows_sales($r_start_date,$r_end_date)
	{
		$r_start_date1 = strtotime($r_start_date);
		$r_end_date1 = strtotime($r_end_date);
		$s_date = date("Y-m-d", $r_start_date1);
		...
		//print_f($s_date);
		//print_f($e_date);
		$this->where('showtime_date', array($s_date,$e_date), 'BETWEEN');
		$this->select(array('SUM(showtimes.show_sales)' => 'perday_sale'));	  	
		$this->from('showtimes');
		return $this->result();
	}
