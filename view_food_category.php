		...
		<div class="row">
			<div class="col-md-12">
				<div class="form-header-inner">
					...
					<a class="pull-right add-button" href="add_food_category.php">Add Category</a>
				</div>
			</div><!--col-md-12-->
		</div><!--row-->
			...
			<div class="col-md-12">	
				...
				$results = $food_category->get_food_categories();
				if ($results) {
				?>
				<table border="1" cellpadding="0" cellspacing="0" id="example" class="table table-hover tableView admin-table">
					...
					<tbody class="searchable">
						...
						echo '<tr>';
						echo '<td>'. $res->food_category_name .'</td>';
						echo '<td class="alignCenter"><a class="edit_btn" href="add_food_category.php?id='.$res->food_category_id.'">Edit</a></td>';
						echo '<td class="alignCenter"><input type="hidden" class="id_to_delete" value="'.$res->food_category_id.'"><a href="#" data-toggle="modal" data-target=".delete_confirm_modal"><img class="img_delete" src="assets/images/delete_icon.png"/></a></td>';
						echo '</tr>';
						}
						?>
					</tbody>
...
$(".container").on('click', '.img_delete',function() {
	...
	$(".modal-footer").on('click', '.btn_yes',function() {
		...
		$.post('ajax.php', {'to_delete': id_to_delete, 'action': 'delete_record_fcategory'}, function(data) {
			console.log(data);
			if(data == true){
				location.reload();
			}else{
