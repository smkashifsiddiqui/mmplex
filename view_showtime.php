...
<div class="row form-header" style="margin-bottom: 9px;">
		...
		<div class="row">
			<div class="col-md-12">
				<div class="form-header-inner">
					...
					<a class="pull-right add-button" href="add_showtime.php">Add Show time </a>
				</div>
			</div><!--col-md-12-->
		</div><!--row-->
	...
	<div class="row">
			...
			<div class="col-md-12">	
				...
				$results = $showtime->get_public_showtimes();
				if ($results) {
				?>
				<table border="1" cellpadding="0" id="example" cellspacing="0" class="table table-hover tableView admin-table">
					...
					<tbody class="searchable">
						...
						echo '<td class="alignCenter"><a class="edit_btn" href="edit_showtime.php?id='.$res->showtime_id.'">Edit</a></td>';
...
$(".container").on('click', '.img_delete',function() {
	...
	$(".modal-footer").on('click', '.btn_yes',function() {
		...
		$.post('ajax.php', {'to_delete': id_to_delete, 'action': 'delete_record_showtime'}, function(data) {
			console.log(data);
			if(data == true){
				location.reload();
			}else{
