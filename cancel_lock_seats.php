...
<div class="row form-header">
        ...
        <div class="col-md-2" style="margin-left: 50px;">
            <div class="search-btn input-group">
                <select name="selection" id="selection" class="form-control search-control">
                    <option value="counts">Search by Counts</option>
                    <option value="seat_no">Search by Seat No</option>
                    <option value="lockdate">Search by Date</option>
                </select>
            </div>
        </div>
        <div class="col-md-3">
            <div class="search-btn input-group">
                <input type="text" name="searchFilter" id="searchFilter" class="form-control search-control" placeholder="Display Number of records.">
                <span class="input-group-btn">
            ...
            if($_POST['searchFilter'] != '' || $_POST['searchFilter'] != 0) {
                ...
                if ($results) {
                    ...
                    if(in_array('cancel_ticket', $cancel_capability)) { ?>
                            ...
                            <?php
                            $booking = new booking();
                            $results = $booking->all_locked_seats($_POST['max_record']);
        ...
        if(isset($_POST['selection']) && $_POST['selection'] == 'seat_no') {

            if($_POST['searchFilter'] != '' || $_POST['searchFilter'] != 0) {
                $_POST['seat_no'] = $_POST['searchFilter'];
                $results = $_SESSION['user'];
                if ($results) {
                    $cancel_id = $results->id;
                    $cancel_capability = $results->capabilities;
                    ...
                    if(in_array('cancel_ticket', $cancel_capability)) { ?>
                        ...
                        <?php
                        $booking = new booking();
                        $results = $booking->locked_seats_by_seatno($_POST['seat_no']);
        ...
        if(isset($_POST['selection']) && $_POST['selection'] == 'lockdate') {

            if($_POST['searchFilter'] != '' || $_POST['searchFilter'] != 0) {
                $_POST['lockdate'] = $_POST['searchFilter'];
                $results = $_SESSION['user'];
                if ($results) {
                    $cancel_id = $results->id;
                    $cancel_capability = $results->capabilities;
                    ...
                    if(in_array('cancel_ticket', $cancel_capability)) { ?>
                        ...
                        <?php
                        $booking = new booking();
                        $results = $booking->all_locked_seats_by_date($_POST['lockdate']);
        ...
        <script>
                ...
                $('#selection').change(function () {
                    ...
                    if($(this).val()=='counts') {
                        ...
                        $('#searchFilter').attr("placeholder", "Display Number of records.");
                    }
                    if($(this).val()=='seat_no') {

                        $('#searchFilter').attr("placeholder", "Search by Seat Number");
                    }
                    if($(this).val()=='lockdate') {

                        $('#searchFilter').attr("placeholder", "Search by Date eg. '2017-12-31'");
                    }
